...
def register():
    if request.method == 'POST':
        ...
        if not all([username, email, password, dob, employment, residence_proof, id_front, id_back, banking_letter, bank_statement]):
            return jsonify({'error': 'All documents required: ID photos, residence proof, banking letter, and bank statement'}), 400
        
        # Banking documents size limit
        if len(banking_letter) > 2000000:  # 2MB limit
            return jsonify({'error': 'Banking letter PDF too large (max 2MB)'}), 400
        if len(bank_statement) > 2000000:  # 2MB limit
            return jsonify({'error': 'Bank statement PDF too large (max 2MB)'}), 400
